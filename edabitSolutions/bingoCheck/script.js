...
const bingoCheck = (arr) => {
  ...
  arr.map((subArr) => {
    subArr.map((index) =>
      index === "x" ? marked.push(subArr.indexOf(index)) : null
    );
  }); //
  if (marked.length !== 5) return false;
  if (marked.filter((x, i, arr) => arr.indexOf(x) == i).length === 1)
    return true;
  if (
    marked
      .map((x, i) => (x === diagFor[i] ? true : false))
      .reduce((a, b) => a && b)
  )
    return true;
  if (
    marked
      .map((x, i) => (x === diagRev[i] ? true : false))
      .reduce((a, b) => a && b)
  )
    return true;
  return marked;
